import { paymentMiddleware } from 'x402-express';
import cors from 'cors';

// Enable CORS for widget embedding
const corsMiddleware = cors({
  origin: true,
  credentials: true
});

// In-memory storage for payment sessions
const paymentSessions = new Map();

export default async function handler(req, res) {
  // Handle CORS preflight
  if (req.method === 'OPTIONS') {
    return res.status(200).end();
  }

  // Only allow POST requests
  if (req.method !== 'POST') {
    return res.status(405).json({ error: 'Method not allowed' });
  }

  try {
    // Apply CORS
    await new Promise((resolve) => corsMiddleware(req, res, resolve));

    const { serviceId, amount = 1, description, walletAddress } = req.body;

    if (!serviceId || !walletAddress) {
      return res.status(400).json({
        error: 'Missing required fields: serviceId, walletAddress'
      });
    }

    const sessionId = `service_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;

    // Store session data
    paymentSessions.set(sessionId, {
      type: 'service_payment',
      serviceId,
      amount,
      description,
      walletAddress,
      timestamp: Date.now(),
      status: 'pending'
    });

    // Configure payment middleware
    const config = {
      address: walletAddress, // Use the provided wallet address
      facilitatorUrl: process.env.FACILITATOR_URL || 'https://x402.org/facilitator',
      network: process.env.NETWORK || 'base-sepolia',
      maxValue: BigInt(1000 * 10 ** 6), // Allow up to $1000 USDC
      sessionId
    };

    // Apply x402 payment middleware
    await new Promise((resolve, reject) => {
      paymentMiddleware(config)(req, res, (err) => {
        if (err) reject(err);
        else resolve();
      });
    });

    // Update session status
    if (paymentSessions.has(sessionId)) {
      paymentSessions.get(sessionId).status = 'completed';
    }

    return res.status(200).json({
      success: true,
      message: 'Service payment processed successfully',
      sessionId,
      serviceId,
      amount,
      walletAddress
    });

  } catch (error) {
    console.error('Service payment error:', error);
    return res.status(500).json({
      error: 'Service payment processing failed',
      details: error.message
    });
  }
} 